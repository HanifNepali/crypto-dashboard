...
export function toOrderParam(sortKey: SortKey, direction: SortDirection): string {
  if (sortKey === 'volume') return direction === 'asc' ? 'volume_asc' : 'volume_desc';
  ...
  if (sortKey === 'rank') return direction === 'asc' ? 'market_cap_desc' : 'market_cap_asc';
